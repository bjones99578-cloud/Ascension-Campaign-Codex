...
- `scripts/dev_preview.sh {start|stop|restart|reset|status}` boots/tears down a local preview server at a **fixed** port (5055) and DB/uploads path (`/tmp/ascension-codex-preview/`) — never the real `wiki.db`. Deliberately fixed (not a fresh ad hoc temp path per session) so the exact same command is safely allowlist-able across sessions instead of prompting for permission every time. Creates `venv/` on first run if it doesn't exist yet. Use `reset` to wipe seeded test data between runs, `stop` when done.
